...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_by_user_id_single(self, db: Session, user_id: str) -> Optional[ModelType]:
        return (
            db.query(self.model)
            .filter(self.model.created_by == user_id, self.model.is_deleted == false())
            .first()
        )

    def get_by_user_id(
        self, db: Session, *, user_id: str, page: int = 1, per_page: int = 10
    ) -> List[ModelType]:
        return (
            db.query(self.model)
            .filter(self.model.created_by == user_id, self.model.is_deleted == false())
    ...
    def get_constant(self, db: Session, id: Any) -> Optional[ModelType]:
        return (
            db.query(self.model)
            .filter(self.model.id == id)
            .first()
    ...
    def get_multi_constant(self, db: Session) -> List[ModelType]:
